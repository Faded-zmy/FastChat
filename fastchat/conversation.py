...
def get_conv_template(name: str) -> Conversation:
    ...
    #liwei 当不存在的时候，用vicuna finetune一个
    if name not in conv_templates:
        conv_templates[name]=Conversation(
                    name=name,
                    system="A chat between a curious user and character: {}. The character gives answer that fit his role to the user's questions.".format(name),
                    roles=("USER", name.upper()),
                    messages=(),
                    offset=0,
