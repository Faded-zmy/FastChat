...
def run_eval(lora_path, base_model_path, model_id, question_file, answer_file, num_gpus, cat, max_token):
    # split question file into num_gpus files
    ques_jsons = json.load(open(question_file,"r"))
    # print(ques_jsons)
    save_root=os.path.dirname(answer_file)
    if not os.path.exists(save_root):
        os.makedirs(save_root)
    with open(os.path.expanduser(answer_file), "w") as fout:
        ans_json = {"question_id": "None"}
...
def get_model_answers(lora_path, base_model_path, model_id, question_jsons, answer_file, cat, max_token):
    disable_torch_init()
    ...
    for i, ques_json in enumerate(tqdm(question_jsons)):
        ...
        for j, sentence in tqdm(enumerate(conversations)):
            ...
            if j % 2 == 0:
                ...
                print("*"*60)
                print("total_ids",len(output_ids[0]))
                print("*"*60)
                if len(output_ids[0]) <= max_token: ## 超过max-token的不输出
